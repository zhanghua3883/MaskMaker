...
class Trainer(BasicTrainer):
    def set_networks(self, opt):
        ...
        # 新增Shapley交互计算模块
        self.shapley_module = ShapleyInteraction()
        
        # 新增交互损失权重参数
        self.lambda_interaction = 0.5  # 需要根据实验调整
    ...
    def forward_seg_train_contrast(self, input_img):
        ...
        self.loss_feature   = feature_loss # .data
         ...
         # 新增：准备Shapley计算需要的任务输出
        task_outputs = {
            'seg': F.softmax(preds[:self._nb], dim=1),          # 仅分割任务
            'seg_recon': F.softmax(preds[:self._nb], dim=1) * \
                        torch.sigmoid(recons_at_prime[:self._nb])  # 联合任务
        }
        
        # 计算交互损失
        self.loss_interaction = self.shapley_module(task_outputs)
        ...
        self.loss_seg = (loss_dice * lambda_dice + 
                        ...
                        domain_loss + 
                        feature_loss + 
                        self.lambda_interaction * self.loss_interaction) * lambda_Seg
        self.loss_seg_tr    = self.loss_seg.data
    ...
    def optimize_parameters_contrast(self, iter, opt):
        ...
        wandb.log({f"con_loss_interaction": self.loss_interaction,
                   f"con_loss_dice": self.loss_dice,
        ...
        if (iter % opt.print_iter_freq == 0):
            print(f"loss_interaction: {self.loss_interaction:.3f}, "
                  f"Contrast Step {iter}, "
...
class ShapleyInteraction:
    ...
    def compute_marginal_contribution(self, coalition_values):
        """计算Shapley值的边际贡献"""
        marginal_contrib = []
        for i in range(self.n_players):
            # 计算包含i和不包含i的联盟差值
            mask = (coalition_values[:, i] != 0)
            contrib = torch.where(mask, coalition_values[:, i] - coalition_values[:, i-1], 0)
            marginal_contrib.append(contrib)
        return torch.stack(marginal_contrib, dim=1)
    
    def __call__(self, task_outputs):
        """
        task_outputs: 包含不同任务组合输出的字典
        返回: 基于Shapley值的交互损失
        """
        # 计算不同联盟的效用值
        coalition_values = torch.stack([
            task_outputs['seg'],          # 仅分割任务
            task_outputs['seg_recon'],     # 分割+重建联合任务
        ], dim=1)
        
        # 计算边际贡献
        marginal_contrib = self.compute_marginal_contribution(coalition_values)
        
        # 计算交互强度 (参考FCP-Net论文公式)
        interaction_strength = torch.abs(marginal_contrib[:, 1] - marginal_contrib[:, 0])
        return interaction_strength.mean()
