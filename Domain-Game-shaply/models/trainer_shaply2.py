...
class Trainer(BasicTrainer):
    def set_networks(self, opt):
        ...
        # 添加变分网络的参数
        self.fc1_mi = nn.Linear(352, 256).to(device)
        self.fc2_mi = nn.Linear(256, 256).to(device)
        self.fc_mu_mi = nn.Linear(256, 352).to(device)
        self.fc_logvar_mi = nn.Linear(256, 352).to(device)
    ...
    def forward_seg_train_contrast(self, input_img):
        ...
        outputs['masks'][:self._nb].detach()
        # 计算交互损失 多通道MSE损失
        seg_alone = outputs['masks'][:self._nb].detach()
        loss_interaction = F.mse_loss(joint_output, seg_alone)
        self.loss_interaction = loss_interaction
        ...
        a_feat = domain_feature_a[-1]  # 最后一层的解剖特征
        d_feat = domain_feature_at[-1]  # 最后一层的域特征
        # 全局平均池化减少维度 (B,C,H,W) -> (B,C)
        a_flat = F.adaptive_avg_pool2d(a_feat, (1, 1)).view(a_feat.size(0), -1)
        d_flat = F.adaptive_avg_pool2d(d_feat, (1, 1)).view(d_feat.size(0), -1)

        # 变分网络的前向传播
        x = F.relu(self.fc1_mi(d_flat))
        x = F.relu(self.fc2_mi(x))
        mu = self.fc_mu_mi(x)
        logvar = self.fc_logvar_mi(x)

        # 计算log q(a|d)
        log_q = -0.5 * (logvar + (a_flat - mu).pow(2) / logvar.exp())
        log_q = log_q.sum(dim=1)

        # 计算log p(a)（假设先验为标准正态分布）
        log_p = -0.5 * (a_flat.pow(2)).sum(dim=1)
    ...
    def optimize_parameters_contrast(self, iter, opt):
        ...
        wandb.log({"con_loss_dice": self.loss_dice,
                   ...
                   f"loss_interaction": self.loss_interaction
                   })
