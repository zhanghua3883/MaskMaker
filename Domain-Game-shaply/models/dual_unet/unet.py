...
class InteractionBranch(nn.Module):
    def __init__(self, seg_channels=4, recon_channels=1):
        super().__init__()
        # 通道对齐层
        self.recon_adjust = nn.Conv2d(recon_channels, seg_channels, kernel_size=1)
        # 交互处理模块
        self.interaction_conv = nn.Sequential(
            nn.Conv2d(seg_channels, 32, kernel_size=1, bias=False),
            nn.BatchNorm2d(32),
            nn.ReLU(inplace=True),
            nn.Conv2d(32, seg_channels, kernel_size=1)  # 保持与分割输出相同通道
        )

    def forward(self, seg_output, recon_output):
        """
        输入:
            seg_output: [B, C, H, W] 分割输出 (C=类别数)
            recon_output: [B, 1, H, W] 重建输出
        输出:
            joint_output: [B, C, H, W] 联合特征
        """
        # 通道对齐
        recon_adjusted = self.recon_adjust(recon_output)  # [B, C, H, W]
        # 特征融合
        fused = seg_output + recon_adjusted
        # 交互处理
        return self.interaction_conv(fused)
...
class EfficientUNet(torch.nn.Module):
    ...
    def forward(self, x, volatile_return_feature=False, enc_features=None,
        ...
        if dual:
            ...
            outputs['anatomical'], outputs['anatomical_comp'] = self.model.reconstruction_head(self.decoder_a,
                                                                                               return_comp=True)
        ...
        outputs['masks'] = masks
        if dual and D:# 添加交互处理
            seg_output = outputs['masks'][:self.nb]
            recon_output = outputs['a_prime_mix'][:self.nb]
            outputs['joint_output'] = self.interaction_branch(seg_output, recon_output)
        return outputs
